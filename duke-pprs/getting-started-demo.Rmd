...
This is an R Markdown document. There are two keys to your use of it:
...
1.  First, be sure that you are viewing the document in the "Visual
    ...
    visible or apparent.
2.  Second, click "Knit" at the top of this screen to preview the
...
that we'll be using is the tidyverse.

#### tidyverse 📦

![](img/tidyverse.png){width="40%"}

The {tidyverse} package is actually a [collection of R
...
block-or "chunk"-of code that follows.
...
Last, let's check that the code worked as we intended; type
`sci_classes` into the code chunk below and run the next chunk and look
at the results.
...
**Note:** You can tab left or right with the arrows, or scan through the
rows by clicking the numbers at the bottom of the pane with the
print-out of the data you loaded.
...
```{r}
ggplot(sci_classes, aes(x = FinalGradeCEMS)) +
  geom_histogram()
```
...
```{r}
m1 <- lm(FinalGradeCEMS ~ TimeSpent + subject, data = sci_classes)
summary(m1)
```
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent + subject, data = sci_classes)
summary(m2)
```
